...
Chuck ported his programs to other platforms of the era, e.g. VAX/VMS, but didn't always combine the changes into the source code for the UNIX programs. Chuck also created versions of his file transfer programs for MS-DOS/PC-DOS computers. Combined into a single executable program capable of either sending or receiving files, he released several variants for DOS: [DSZ.COM](http://DSZ.COM), DSZ.EXE, GSZ.EXE, and FDSZ.EXE.  These programs were released without source code under a "shareware" license and were very popular among BBS operators of the era.
